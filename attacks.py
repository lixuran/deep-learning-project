...
class Attacks(object):
    ...
    def pgd_untargeted(self, x, label, k, eps, eps_step, is_fc=True, clip_min=None, clip_max=None, **kwargs):
        return self.pgd_(x, label, k, eps, eps_step, targeted=False, is_fc=is_fc, clip_min=clip_min, clip_max=clip_max,
                         **kwargs)
    ...
    def VAEA(self, x, target, chosen_norm, steps, eps, eps_norm, step_size=0.01, clamp=(0, 1), delta=0.1):
        ...
        #recon_t.clamp_(min=clamp[0], max=clamp[1])
        output = x_star
        #print(x_star.shape)
        ...
        for step in range(steps):
            ...
            recon_x_star.clamp_(min=clamp[0], max=clamp[1])
            #print(recon_x_star.shape)
            #print(recon_t.shape)
            recon_x_star = recon_x_star / max(torch.max(recon_x_star),1)
            BCE = F.binary_cross_entropy(recon_x_star, recon_t, size_average=False)
            KLD = -0.5 * torch.sum(1 + logvar - mu ** 2 - logvar.exp())
            loss_2 = torch.nn.MSELoss()  # Changed (TE)  % todo: change to binary cross entropy loss
    ...
    def LatentA(self, x, target, chosen_norm, steps, eps, eps_norm, step_size=0.01, clamp=(0, 1), delta=0.1):
         # Think this should be initialized (Tobias)
        x_star = x.clone().detach_()
        target_ = target.clone().detach_()
        # ... and make sure we are differentiating toward that variable
        x_star = flatten(x_star)
        ...
        for step in range(steps):
            ...
            loss_2 = torch.nn.MSELoss() #todo: change to BCE loss
            loss = delta * loss_2(flatten(x), x_star) + loss_2(enc_x_star, z_t)
            loss.backward()
            ...
            # projection
            if eps_norm == 'infinite':
                output = torch.max(torch.min(output, flatten(x) + eps), flatten(x) - eps)
            else:
                diff = x_star - x
