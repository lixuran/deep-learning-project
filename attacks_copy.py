...
class Attacks(object):
    ...
    def VAEA(self, model, x, target, chosen_norm, steps, eps, eps_norm, step_size=0.01, clamp=(0,1), delta= 0.1):
    ...
    def LatentA(self, model, x, target, chosen_norm, steps, eps, eps_norm,step_size=0.01, clamp=(0,1), delta= 0.1):
        ...
        z_t = model.latent(target_)[0]
        z_t.requires_grad_(False)
        # use PGD to create adverserial image
        for step in range(steps):
            x_star = output.clone().detach_()
            x_star.requires_grad_()
            enc_x_star = model.latent(x_star)[0]
